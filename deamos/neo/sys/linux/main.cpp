...
bool Sys_GetPath(sysPath_t type, idStr &path) {
	...
	case PATH_CONFIG:
		...
		if (s)
			...
			idStr::snPrintf(buf, sizeof(buf), "%s/.config/deamos", getenv("HOME"));

		path = buf;
		return true;

	case PATH_SAVE:
		s = getenv("HOME");//getenv("XDG_DATA_HOME");//angelo path
		if (s)
			idStr::snPrintf(buf, sizeof(buf), "%s/deamos", s);
		else
			idStr::snPrintf(buf, sizeof(buf), "%s/.local/share/deamos", getenv("HOME"));
