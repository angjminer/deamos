...
public:
	...
	int     				lastSnapshotSequence;	// track state hitches on clients
...
public:
  ...
  // Nicemice: Event_Remove ensures that map entities aren't removed
  // which are spawned on the server and on the client BUT
  // are not networksynced
  void          Event_Remove();
  void          Event_SafeRemove();
  
  virtual bool NetworkSyncTarget() const { return false; };

  // Nicemice: deprecated
  bool RequestSnapshot(int clientNum);
  void SetSnapshotRequest(int clientNum);
  void ResetSnapshotRequest(int clientNum);
  bool requestSnapshot[MAX_CLIENTS];
...
protected:
  ...
  // Nicemice: Server syncing
  bool IsEventSender() const;
  bool IsEventReceiver() const;

  // Nicemice: Are Entity events getting syncronised with the server?
  bool isServerSyncronised;
...
public:
  ...
  // Nicemice: added
  virtual bool NetworkSyncTarget() const { return true; };
