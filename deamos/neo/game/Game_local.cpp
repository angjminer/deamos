...
void idGameLocal::LocalMapRestart( ) {
  ...
  // Nicemice: fix for not reseting the camera
  // when restarting the server
  // AO: gives an assertion when playing team dm
  // SetCamera( NULL );
  ...
  // since it effects deathmatch maps also
	memset( clientEntityStates, 0, sizeof( clientEntityStates ) );//angelo no load
	/*memset( clientPVS, 0, sizeof( clientPVS ) );
	memset( clientSnapshots, 0, sizeof( clientSnapshots ) );*/
...
/*
================
idGameLocal::HandleESC - Nicemice: modified
================
*/
...
void idGameLocal::ShowTargets( void ) {
  ...
  gameLocal.Warning("coop: idGameLocal::ShowTargets unhandled");
  idPlayer* player = GetLocalPlayer();
  if ( !player )
    return; 
...
idEntity *idGameLocal::SpawnEntityType( const idTypeInfo &classdef, const idDict *args, bool bIsClientReadSnapshot ) {
  ...
  // Nicemice: DEBUG
  if ( isClient ) {
    // gameLocal.Printf( "spawning entity on client %s\n",classdef.classname );
    if ( !bIsClientReadSnapshot ) {
      Warning("spawning entity on client %s",classdef.classname );
    }
  }
...
void idGameLocal::SpawnMapEntities( void ) {
	...
	for ( i = 1 ; i < numEntities ; i++ ) {
		...
		args = mapEnt->epairs;
    // Nicemice: Lets differ between map entities and "temporary entities"
...
/*
================
idGameLocal::RequirementMet - Nicemice: modified
================
*/
...
void idGameLocal::RandomizeInitialSpawns( void ) {
  // Nicemice: FIXME this function should be gametype dependend
  // move it to multiplayer
	spawnSpot_t	spot;
	int i, j;
	idEntity *ent;
  ...
  // Nicemice: Lets allow info_player_starts for spawning
  bool useInfoPlayerStart = false;  
  if ( !spot.ent )
  {
    common->Warning( "no info_player_deathmatch in map" );
    common->Warning( "falling back to info_player_start" );
    useInfoPlayerStart = true;
	...
	while( spot.ent )
  {
	...
	if ( !initialSpots.Num() ) {
		common->Warning( "no spawn entities marked initial in map" );
...
/*
================
idGameLocal::AllowWeapons
================
*/
bool idGameLocal::WeaponsNotActive() const
{
  if ( gameLocal.isMultiplayer )
    return false;

  return world->spawnArgs.GetBool( "no_Weapons" );
}
