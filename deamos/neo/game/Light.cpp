...
void idLight::Spawn( void ) {
	...
	if ( start_off ) {
    // Nicemice: added
    // Something must turn the light on, so we have to networksync it
    if ( fl.networkSync == false ) {
      fl.networkSync = true;
      gameLocal.DPrintf("coop: Adding networkSync(start_off) to %s(%s)\n",GetType()->classname,name.c_str());
    }
...
/*
================
idLight::WriteToSnapshot - Nicemice: modified
================
*/
...
/*
================
idLight::ReadFromSnapshot - Nicemice: modified
================
*/
