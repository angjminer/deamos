...
/*
=====================
idAI::Event_Activate - Nicemice: modified
=====================
*/
void idAI::Event_Activate( idEntity *activator ) {

  // Nicemice: TEST TEST - do we need to send the activate event?
  if ( IsEventSender() ) {
    ServerSendEvent( EVENT_AI_ACTIVATE, NULL, false, -1 );
  }

  // Nicemice: DEBUG
  //if ( isServerSyncronised ) {
  //  gameLocal.Printf("idAI::Event_Activate: %s(%s)\n",GetType()->classname,name.c_str());
  //}
...
/*
=====================
idAI::Event_CreateMissile - Nicemice: modified
=====================
*/
void idAI::Event_CreateMissile( const char *jointname ) {

  // we are receiving messages from the server, so we aren't
  // allowed to create entities by our own
  if ( IsEventReceiver() )
    return;
...
/*
=====================
idAI::Event_AttackMissile - Nicemice: modified
=====================
*/
void idAI::Event_AttackMissile( const char *jointname )
{

  // Nicemice: Don't allow launch a missile projectile (on client)
  if ( IsEventReceiver() )
    return;
...
/*
=====================
idAI::Event_FireMissileAtTarget - Nicemice: modified
=====================
*/
void idAI::Event_FireMissileAtTarget( const char *jointname, const char *targetname ) {

  // Nicemice: Don't allow missile firing!
  if ( IsEventReceiver() )
    return;
...
/*
=====================
idAI::Event_LaunchMissile - Nicemice: modified
=====================
*/
void idAI::Event_LaunchMissile( const idVec3 &org, const idAngles &ang ) {

  // we are receiving messages from the server, so we aren't
  // allowed to launch missiles by our own
  if ( IsEventReceiver() )
    return;
...
/*
=====================
idAI::Event_AttackMelee - Nicemice: modified
=====================
*/
...
/*
=====================
idAI::Event_BecomeRagdoll - Nicemice: modified
=====================
*/
void idAI::Event_BecomeRagdoll( void ) {
  ...
  if ( IsEventSender() )
  {
    ServerSendEvent( EVENT_AI_BECOMERAGDOLL, NULL, false, gameLocal.localClientNum );
  }
...
void idAI::Event_StopRagdoll( void ) {
  ...
  if ( isServerSyncronised )
  {
    ...
    gameLocal.Warning("coop: idAI::Event_StopRagdoll unhandled");
  }
...
/*
=====================
idAI::Event_PreBurn - Nicemice: modified
=====================
*/
void idAI::Event_PreBurn( void ) {
  ...
  if ( IsEventSender() )
  {
    ServerSendEvent( EVENT_AI_PREBURN, NULL, false, gameLocal.localClientNum );

    //Nicemice: DEBUG
    //gameLocal.Printf("Server: ");
  }

  //Nicemice: DEBUG
  //gameLocal.Printf("idAI::Event_PreBurn\n");
...
void idAI::Event_Burn( void ) {
  ...
  if ( IsEventSender() )
  {
    ServerSendEvent( EVENT_AI_BURN, NULL, false, gameLocal.localClientNum );

    //gameLocal.Printf("Server: ");
  }

  //gameLocal.Printf("idAI::Event_Burn\n");
...
void idAI::Event_ClearBurn( void ) {
  ...
  {
    //Nicemice: Needs to be fixed?!
    gameLocal.Warning("coop: idAI::Event_ClearBurn unhandled");
  }
...
void idAI::Event_RealKill( void ) {
  ...
  {
    //Nicemice: Needs to be fixed?!
    gameLocal.Warning("coop: idAI::Event_RealKill unhandled");
  }
...
void idAI::Event_TriggerParticles( const char *jointName ) {
  ...
  {
    //Nicemice: Needs to be fixed?!
    gameLocal.Warning("coop: idAI::Event_TriggerParticles unhandled");
  }
