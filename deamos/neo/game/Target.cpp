...
void idTarget_EndLevel::Event_Activate( idEntity *activator ) {
  ...
  // Nicemice: added
  if ( gameLocal.isServer ) {
    //const idDeclEntityDef *mapDef = static_cast<const idDeclEntityDef *>( declManager->FindType( DECL_MAPDEF, nextMap.c_str() ) );//angelo map change
    //const idDeclEntityDef *mapDef = ( declManager->FindType( DECL_MAPDEF, nextMap.c_str() ) );//angelo map change    
			//if ( mapDef != 0 ) {
			  if ( nextMap.c_str() != 0 ) {
...
//EVENT( EV_Activate,				idTarget_GiveEmail::ClientReceiveEvent)
END_CLASS
