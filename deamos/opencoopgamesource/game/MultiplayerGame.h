...
//! Define a basic multiplayer mode
/*class idBaseMultiplayer
{
public:
  idBaseMultiplayer();
  ~idBaseMultiplayer();

  //! Shuts down the multiplayer mode
	void Shutdown( void );
  ...
  //! a client disconnected
	...
	//! Sets up local data for a new player - FIXME, ONLY SERVER SIDE?
  ...
  // Game events
	//! Updates frag counts
	virtual void PlayerDeath( idPlayer *dead, idPlayer *killer, bool telefrag );
  ...
  //! client enters the game Nicemice: FIXME, move to Spawnplayer?
  ...
  //static const char *GameStateStrings[ STATE_COUNT ];
	idBaseMultiplayer::gameState_t		GetGameState( void ) const;
...
private:
  ...
  //! Initializes the multiplayer mode
  virtual void Init();

  //! Cleans the multiplayer mode up
  virtual void Clear();
};
*/
