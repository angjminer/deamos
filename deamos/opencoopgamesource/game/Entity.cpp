...
ABSTRACT_DECLARATION( idClass, idEntity )
  ...
  EVENT( EV_SafeRemove,	      idEntity::Event_SafeRemove )
END_CLASS
...
idEntity::idEntity() {
  ...
  isMapEntity = false;
  // Nicemice: added
  // if set to true events are partially synced with the server
  // currently only used for monsters (idAI)
  // i.e.: isServerSyncronised could by replaced with
  // IsType(idAI::Type)
  isServerSyncronised = false;

  // Nicemice: deprecated
  for (unsigned int i = 0; i < MAX_CLIENTS; ++i)
  {
    requestSnapshot[i] = false;
  }
...
/*
===============
idEntity::IsEventSender - Nicemice: added
===============
*/
bool idEntity::IsEventSender() const
{
  // Nicemice: We only want to send animations if we are a
  // pure server or a listen server and
  // isServerSyncronised is set to true.
  if (gameLocal.isMultiplayer && gameLocal.isServer && isServerSyncronised)
    return true;
...
/*
===============
idEntity::IsEventReceiver - Nicemice: added
===============

*/
bool idEntity::IsEventReceiver() const
{
  // Nicemice: We only want to recieve animations if we are
  // a pure client (and not a listen client)
  // and isServerSyncronised is set to true;
  if (gameLocal.isMultiplayer && !gameLocal.isServer)
    if (gameLocal.isClient && isServerSyncronised)
      return true;
...
void idEntity::Spawn( void ) {
    ...
    // Nicemice: added
    // we assume that hidden entities will be activated at some point in time,
    // so lets networksync them
    if ( fl.networkSync == false ) {
      fl.networkSync = true;
      gameLocal.DPrintf("coop: Adding networkSync(hidden) to %s(%s)\n",GetType()->classname,name.c_str());
    }

		PostEventMS( &EV_Hide, 0 );
	}
	cinematic = spawnArgs.GetBool( "cinematic", "0" );
 // Nicemice: do we have a gui ?
  hasGui = spawnArgs.FindKey( "gui" );
  if ( fl.networkSync == false && hasGui ) {
    // we have a gui, is it interactive?
    bool isNonInteractive = false;
    spawnArgs.GetBool("gui_noninteractive", "0",isNonInteractive);
    if ( !isNonInteractive ) {
      // the gui is interactive, lets sync the entity
      gameLocal.DPrintf("coop: Adding networkSync(gui) to %s(%s)\n",GetType()->classname,name.c_str());
      fl.networkSync = true;
    }
  }
  ...
  spawnArgs.GetBool( "mapEntity", "0", isMapEntity );
    ...
    // Nicemice: added
    // We assume that entities which are binded to something are moving entities,
    // so we have to networksync them
    if ( fl.networkSync == false ) {
      fl.networkSync = true;
      gameLocal.DPrintf("coop: Adding networkSync(bind) to %s(%s)\n",GetType()->classname,name.c_str());
    }
...
idEntity::~idEntity( void ) {
  ...
  gameLocal.DPrintf("removing entity %i: %s(%s)\n", entityNumber, GetType()->classname, entityname.c_str());*/
...
bool idEntity::HandleGuiCommands( idEntity *entityGui, const char *cmds ) {
	...
	if ( entityGui && cmds && *cmds ) {
		...
		while( 1 ) {
          ...
          //Nicemice: Needs to be fixed?!
          gameLocal.Warning("coop: idEntity::HandleGuiCommands unhandled");
          idPlayer* player = gameLocal.GetLocalPlayer();
          if ( player ) {
					  if ( score >= 25000 && !player->inventory.turkeyScore ) {
						  player->GiveEmail( "highScore" );
						  player->inventory.turkeyScore = true;
					  }
          }
				}
				continue;
...
void idEntity::FindTargets( void ) {
    ...
    // Nicmice: added
    // we ensure that entities which are targeted by this one are networksynced
    // if needed
    if ( ent->isMapEntity && ent->fl.networkSync == false)
    {
      if ( ent->NetworkSyncTarget() || IsType( idTarget_Remove::Type ) )
      {
        ent->fl.networkSync = true;
        gameLocal.DPrintf("coop: Adding networkSync(target) to %s(%s)\n",ent->GetType()->classname,ent->name.c_str());
      }
    }
	}
}
...
/*
==============================
idEntity::ActivateTargets - Nicemice: modified
...
void idEntity::ActivateTargets( idEntity *activator ) const {
	...
	// Nicemice: TEST TEST - allow activating targets on client? no!
  if ( gameLocal.isClient )
    return;
...
void idEntity::Event_SetShaderParm( int parmnum, float value ) {
  if ( gameLocal.isServer ) {
    ...
    msg.WriteShort( parmnum );
    msg.WriteFloat( value );
    ServerSendEvent( EVENT_SETSHADERPARM, &msg, false, -1 );
  }
  // Nicemice: DEBUG
  // gameLocal.DPrintf( "idEntity::Event_SetShaderParm called\n" );
...
/*
================
idEntity::Event_Hide - Nicemice: modified
================
*/
void idEntity::Event_Hide( void ) {

  if ( gameLocal.isServer ) {
    ServerSendEvent( EVENT_HIDE, NULL, false, -1 );
  }
...
/*
================
idEntity::Event_Show - Nicemice: modified
================
*/
void idEntity::Event_Show( void ) {

  if ( gameLocal.isServer ) {
    ServerSendEvent( EVENT_SHOW, NULL, false, -1 );
  }
...
void idEntity::Event_Remove()
{
  // Nicemice: We don't allow EV_REMOVE to remove an entity 
  // if it is a mapentity and we are in multiplayer mode
  if (gameLocal.isMultiplayer && fl.networkSync == false && isMapEntity)
  {
    idStr entityname;
    spawnArgs.GetString("name", "", entityname );   
    gameLocal.Warning("coop: enitiy %i: %s was not removed. networkSync enabled ?", entityNumber, entityname.c_str());
    return;
  }
...
void idEntity::Event_SafeRemove()
{
...
void idEntity::WriteGUIToSnapshot( idBitMsgDelta &msg ) const {
	...
	if ( renderEntity.gui[ 0 ] ) {
		msg.WriteLong( renderEntity.gui[ 0 ]->State().GetInt( "networkState" , "-1" ) );
    // Nicemice: DEBUG
    //gameLocal.DPrintf("networkstate: %i\n",renderEntity.gui[ 0 ]->State().GetInt( "networkState" ));
	} else {
		msg.WriteLong( 0 );
	}
}
...
void idEntity::ReadGUIFromSnapshot( const idBitMsgDelta &msg ) {
    ...
    // Nicemice: DEBUG
    //gameLocal.DPrintf("gui networkstate: %i\n",renderEntity.gui[ 0 ]->State().GetInt( "networkState" ));
...
/*
================
idEntity::WriteToSnapshot - Nicemice: modified
================
*/
...
/*
================
idEntity::ReadFromSnapshot - Nicemice: modified
================
*/
...
void idEntity::ServerSendEvent( int eventId, const idBitMsg *msg, bool saveEvent, int excludeClient ) const {
  ...
  // Nicemice: DEBUG
  //gameLocal.Printf( "Event: %i %i %s(%s)\n",eventId,entityNumber,GetType()->classname,name.c_str());
...
bool idEntity::RequestSnapshot(int clientNum)
{
  if (clientNum < 0 || clientNum >= MAX_CLIENTS )
    return false;

  if ( gameLocal.isClient ) {
    if ( !requestSnapshot[clientNum] ) {
      
      idBitMsg     msg;
      byte msgBuf[MAX_EVENT_PARAM_SIZE];
      msg.Init( msgBuf, sizeof( msgBuf ) ); 
      msg.WriteShort( clientNum );
      ClientSendEvent( EVENT_REQUESTSNAPSHOT, &msg );
      /*idStr entityname;
      spawnArgs.GetString("name", "", entityname );   
      gameLocal.DPrintf("coop:snapshotrequest %i: %s(%s)\n", entityNumber, GetType()->classname, entityname.c_str());*/

      SetSnapshotRequest(clientNum);
      return true;
    }
    //we have requested a snapshot already
  } else if ( gameLocal.isServer ) {
    if ( requestSnapshot[clientNum] ) {
      ResetSnapshotRequest(clientNum);
      // a snapshot is requested, lets send it
      /*idStr entityname;
      spawnArgs.GetString("name", "", entityname );   
      gameLocal.DPrintf("coop:snapshotrequest %i: %s(%s)\n", entityNumber, GetType()->classname, entityname.c_str());*/
      return true;
...
void idEntity::SetSnapshotRequest(int clientNum)
{
  if (clientNum < 0 || clientNum >= MAX_CLIENTS )
    return;

  requestSnapshot[clientNum] = true;
}

void idEntity::ResetSnapshotRequest(int clientNum)
{
  if (clientNum < 0 || clientNum >= MAX_CLIENTS )
    return;

  requestSnapshot[clientNum] = false;
}
...
void idEntity::ReadHiddenFromSnapshot( const idBitMsgDelta &msg ) {
	...
	if ( hidden != IsHidden() ) {
    gameLocal.Warning("Hidden state hitch on entity %d: %d %s(%s)", entityNumber, entityDefNumber, GetType()->classname, name.c_str() );
		if ( hidden ) {
			Hide();
		} else {
			Show();
...
bool idEntity::ServerReceiveEvent( int event, int time, const idBitMsg &msg ) {
  ...
  switch (event)
  {
  case EVENT_REQUESTSNAPSHOT: {
    int clientNum = msg.ReadShort();
    SetSnapshotRequest(clientNum);
    return true;
  }
  };
...
/*
================
idEntity::ClientReceiveEvent - Nicemice: modified
================
*/
bool idEntity::ClientReceiveEvent( int event, int time, const idBitMsg &msg ) {
    ...
    case EVENT_SETSHADERPARM: {
      int parmnum = msg.ReadShort();
      float value = msg.ReadFloat();
      Event_SetShaderParm( parmnum, value );
      return true;
      }
		default: {
			return false;
...
void idAnimatedEntity::Event_ClearAllJoints( void ) {
  // Nicemice: DEBUG
  if ( isServerSyncronised )
    gameLocal.Printf("idAnimatedEntity::Event_ClearAllJoints: %s(%s)\n",GetType()->classname,name.c_str());
...
void idAnimatedEntity::Event_ClearJoint( jointHandle_t jointnum ) {
  // Nicemice: DEBUG
  if ( isServerSyncronised )
    gameLocal.Printf("idAnimatedEntity::Event_ClearJoint: %s(%s)\n",GetType()->classname,name.c_str());
...
void idAnimatedEntity::Event_SetJointPos( jointHandle_t jointnum, jointModTransform_t transform_type, const idVec3 &pos ) {
  ...
  // Nicemice: DEBUG
  if ( isServerSyncronised )
    gameLocal.Printf("idAnimatedEntity::Event_SetJointPos: %s(%s)\n",GetType()->classname,name.c_str());
...
void idAnimatedEntity::Event_SetJointAngle( jointHandle_t jointnum, jointModTransform_t transform_type, const idAngles &angles ) {
  ...
  // Nicemice: DEBUG
  if ( isServerSyncronised )
    gameLocal.Printf("idAnimatedEntity::Event_SetJointAngle: %s(%s)\n",GetType()->classname,name.c_str());
