...
bool idInventory::Give( idPlayer *owner, const idDict &spawnArgs, const char *statname, const char *value, int *idealWeapon, bool updateHud ) {
	...
	if ( !idStr::Icmpn( statname, "ammo_", 5 ) ) {
		...
		for( pos = value; pos != NULL; pos = end ) {
			...
			if ( !gameLocal.WeaponsNotActive() || ( weaponName == "weapon_fists" ) || ( weaponName == "weapon_soulcube" ) ) {
...
bool idPlayer::BalanceTDM( void ) {
  ...
  // Nicemice: short and dirty hack to let the players start in team 0
  balanceTeam = 0;
...
/*
===============
idPlayer::GiveSecurity - Nicemice: modified
===============
*/
...
void idPlayer::GivePDA( const char *pdaName, idDict *item )
{
	...
	if ( gameLocal.GetFrameNum() > 10 ) {
      ...
      // Nicemice: only show the tip if we are in singpleplayer mode
      if ( !gameLocal.isMultiplayer ) {
        if ( objectiveSystem ) {
...
void idPlayer::NextWeapon( void ) {
	...
	if ( !weaponEnabled || spectating || hiddenWeapon || gameLocal.inCinematic || gameLocal.WeaponsNotActive() || health < 0 ) {
		return;
	}
...
void idPlayer::SelectWeapon( int num, bool force ) {
	...
	if ( ( num != weapon_pda ) && gameLocal.WeaponsNotActive() ) {
		num = weapon_fists;
		hiddenWeapon ^= 1;
...
void idPlayer::UpdateFocus( void ) {
  ...
  // Nicemice: allow focus in multiplayer, FIXME: make it gamemode dependend
...
void idPlayer::TogglePDA( void ) {
  bool tempObjectiveSystemOpen = objectiveSystemOpen;
...
/*
==============
idPlayer::HandleESC - Nicemice: modified
==============
*/
...
/*
==============
idPlayer::EvaluateControls - Nicemice: modified
==============
*/
void idPlayer::EvaluateControls( void ) {
  ...
  // Nicemice: Openening the scoreboard should be gametype dependend
...
void idPlayer::Event_EnableWeapon( void ) {
	hiddenWeapon = gameLocal.WeaponsNotActive();
	weaponEnabled = true;
	if ( weapon.GetEntity() ) {
		weapon.GetEntity()->ExitCinematic();
	}
...
void idPlayer::Event_GetPreviousWeapon( void ) {
	...
	if ( previousWeapon >= 0 ) {
		int pw = ( gameLocal.WeaponsNotActive() ) ? 0 : previousWeapon;
...
void idPlayer::Event_SelectWeapon( const char *weaponName ) {
	...
	if ( hiddenWeapon && gameLocal.WeaponsNotActive() ) {
...
/*
===============
idPlayer::ShowTip - Nicemice: modified
===============
*/
