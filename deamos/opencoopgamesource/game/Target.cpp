...
/*
================
idTarget_Remove::Event_Activate - Nicemice: modified
================
*/
...
//#endif
  ...
  // Nicemice: added
  if ( gameLocal.isServer ) {
    const idDeclEntityDef *mapDef = static_cast<const idDeclEntityDef *>( declManager->FindType( DECL_MAPDEF, nextMap.c_str() ) );
			if ( mapDef != 0 ) {
 			  si_map.SetString( nextMap );
			  gameLocal.MapRestart();
      } else {
        gameLocal.Warning("target_endlevel: map %s not found on server",nextMap.c_str() );
      }
  }
}
...
void idTarget_Give::Event_Activate( idEntity *activator ) {
  ...
  if ( activator && activator->IsType( idPlayer::Type )) {
    ...
    // Nicemice: The activator is not a player and we are in multiplayer
    // mode, who should get the weapons?
    gameLocal.Warning("coop: idTarget_Give::Event_Activate unhandled");
...
void idTarget_Give::Give(idPlayer *player) {
  
  if ( !player || gameLocal.isClient )
    return;
...
/*
================
idTarget_GiveEmail::ClientReceiveEvent - Nicemice: added
================
*/
bool idTarget_GiveEmail::ClientReceiveEvent(int event,int time,const idBitMsg &msg)
{
  switch (event)
  {
  case EVENT_GIVEEMAIL: {
    Event_Activate(NULL);
    return true;
    }
  };
  return idTarget::ClientReceiveEvent(event,time,msg);
}

/*
================
idTarget_GiveEmail::Event_Activate - Nicemice: modified
================
*/
void idTarget_GiveEmail::Event_Activate( idEntity *activator ) {
  ...
  if ( gameLocal.isServer )
  {
    ServerSendEvent( EVENT_GIVEEMAIL, NULL, false, -1 );
  }
...
/*
================
idTarget_SetInfluence::ClientReceiveEvent - Nicemice: added
================
*/

void idTarget_SetInfluence::ClientPredictionThink()
{
...
================
*/
bool idTarget_SetInfluence::ClientReceiveEvent(int event,int time,const idBitMsg &msg)
{
  switch(event)
  {
  case EVENT_ACTIVATE: {
  ...
  case EVENT_CLEARFLASH: {
    float flash = msg.ReadFloat();
    Event_ClearFlash(flash);
    return true;
    }
  case EVENT_FLASH: {
    float flash = msg.ReadFloat();
    int out = msg.ReadLong();
    Event_Flash(flash,out);
    return true;
    }
  case EVENT_RESTORE: {
...
/*
================
idTarget_SetInfluence::Event_Flash - Nicemice: modified
================
*/
void idTarget_SetInfluence::Event_Flash( float flash, int out ) {
  ...
  if ( gameLocal.isServer )
  {
    ...
    msg.WriteFloat(flash);
    msg.WriteLong(out);
    ServerSendEvent( EVENT_FLASH, &msg, false, -1 );
  }
...
/*
================
idTarget_SetInfluence::Event_ClearFlash - Nicemice: modified
================
*/
void idTarget_SetInfluence::Event_ClearFlash( float flash ) {
  ...
  if ( gameLocal.isServer )
  {
    ...
    msg.WriteFloat(flash);
    ServerSendEvent( EVENT_CLEARFLASH, &msg, false, -1 );
  }
...
/*
================
idTarget_SetInfluence::Event_Activate - Nicemice: modified
================
*/
...
void idTarget_SetInfluence::Event_RestoreInfluence() {
	...
	if ( gameLocal.isServer )
  {
    ServerSendEvent( EVENT_RESTORE, NULL, false, -1 );
  }
...
void idTarget_SetFov::Think( void ) {
  //Nicemice: Needs to be fixed?!
  gameLocal.Warning("coop: idTarget_SetFov::Think unhandled");
...
/*
================
idTarget_SetPrimaryObjective::ClientReceiveEvent
================
*/
bool idTarget_SetPrimaryObjective::ClientReceiveEvent(int event,int time,const idBitMsg &msg)
{
  switch (event)
  {
  case EVENT_SETPRIMARYOBJ: {
    Event_Activate(NULL);
    return true;
    }
  };
...
void idTarget_SetPrimaryObjective::Event_Activate( idEntity *activator ) {
  ...
  if ( gameLocal.isServer )
  {
      // Should we store the event on the server? 
   		ServerSendEvent( EVENT_SETPRIMARYOBJ, NULL, false, -1 );     

      // Nicemice: DEBUG
      //gameLocal.DPrintf("Server: ");
  }

  //gameLocal.DPrintf("idTarget_SetPrimaryObjective\n");
...
/*
================
idTarget_Tip::idTarget_Tip - Nicemice: modified
================
*/
...
/*
================
idTarget_Tip::ClientRecieveEvent
================
*/
bool idTarget_Tip::ClientReceiveEvent(int event,int time,const idBitMsg &msg)
{
  switch (event)
  {
  case EVENT_TIP_ACTIVATE: {
    Event_Activate(NULL);
    return true;
    }
  };
...
void idTarget_Tip::Event_Activate( idEntity *activator ) {
  ...
  if ( gameLocal.isServer )
  {
    ServerSendEvent(EVENT_TIP_ACTIVATE, NULL, false, -1 );
  }
...
/*
================
idTarget_Tip::Event_TipOff - Nicemice: modified
================
*/
...
/*
================
idTarget_GiveEmail::ClientReceiveEvent
================
*/
bool idTarget_GiveSecurity::ClientReceiveEvent(int event, int time, const idBitMsg &msg)
{
  switch (event)
  {
  case EVENT_GIVESECURITY: {
    Event_Activate( NULL );
    return true;
    }
  };
...
void idTarget_GiveSecurity::Event_Activate( idEntity *activator ) {
  ...
  if ( gameLocal.isServer )
  {
    ServerSendEvent( EVENT_GIVESECURITY, NULL, false, -1 );
  }
