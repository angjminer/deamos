...
/*
================
idExplodable::Spawn - Nicemice: modified
================
*/
...
void idExplodable::Event_Explode( idEntity *activator ) {
  ...
  // only remove it if we are allowed to
  if ( !gameLocal.isClient )
...
/*
===============
idAnimated::ClientReceiveEvent
===============
*/
bool idAnimated::ClientReceiveEvent(int event,int time,const idBitMsg &msg) {
  ...
  case EVENT_ANIMDONE: {
    int animIndex = msg.ReadShort();
    Event_AnimDone(animIndex);
    return true;
    }
  case EVENT_START: {
    Event_Start();
...
void idAnimated::PlayNextAnim( void ) {
	...
	int cycle;
  // Nicemice: DEBUG
  //gameLocal.Printf("idAnimated::PlayNextAnim: %s(%s)\n",GetType()->classname,name.c_str());
...
void idAnimated::Event_StartRagdoll( void ) {
  ...
  if ( gameLocal.isServer )
  {   
    ServerSendEvent( EVENT_STARTRAGDOLL, NULL, false, -1 );
  }
	StartRagdoll();
}
...
void idAnimated::Event_AnimDone( int animindex ) {
  ...
  {   
    ...
    msg.WriteShort(animindex);
    ServerSendEvent( EVENT_ANIMDONE, &msg, false, -1 );
  }
  // Nicemice: DEBUG
  //gameLocal.Printf("idAnimated::Event_AnimDone: %s(%s)\n",GetType()->classname,name.c_str());
...
void idAnimated::Event_Activate( idEntity *_activator ) {
  if ( gameLocal.isServer )
  {
    ...
    ServerSendEvent( EVENT_ACTIVATE, NULL, false, -1 );
  }
  // Nicemice: DEBUG
  //gameLocal.Printf("idAnimated::Event_Activate: %s(%s)\n",GetType()->classname,name.c_str());
...
/*
===============
idAnimated::Event_Start - Nicemice: modified
================
*/
void idAnimated::Event_Start( void ) {
  ...
  /*if ( gameLocal.isServer )
  {   
    ServerSendEvent( EVENT_START, NULL, false, -1 );
  }*/

  // Nicemice: DEBUG
  //gameLocal.Printf("idAnimated::Event_Start: %s(%s)\n",GetType()->classname,name.c_str());
...
/*
================
idStaticEntity::Event_Activate - Nicemice: modified
================
*/
...
/*
================
idStaticEntity::WriteToSnapshot - Nicemice: modified
================
*/
...
/*
================
idStaticEntity::ReadFromSnapshot - Nicemice: modified
================
*/
...
/*
================
idStaticEntity::ClientReceiveEvent - Nicemice: added
================
*/
...
void idFuncEmitter::Spawn( void ) {
	if ( spawnArgs.GetBool( "start_off" ) ) {
		...
		UpdateVisuals();
    // Nicemice: lets turn networksync on so we will be noticed when the "on"
    // event happens
    bool networkSync;
    spawnArgs.GetBool("networkSync","1", networkSync );
    fl.networkSync = networkSync;
...
void idFuncEmitter::Event_Activate( idEntity *activator ) {
  ...
  // Nicemice: UpdateVisuals is done by Event_Show
	UpdateVisuals();
}
...
void idTextEntity::Think( void ) {
    ...
    gameLocal.Warning("coop: idTextEntity::Think unhandled");
    idPlayer* player = gameLocal.GetLocalPlayer();
    if ( player )
      gameRenderWorld->DrawText( text, GetPhysics()->GetOrigin(), 0.25, colorWhite, playerOriented ? player->viewAngles.ToMat3() : GetPhysics()->GetAxis().Transpose(), 1 );
...
void idEarthQuake::Event_Activate( idEntity *activator ) {
	...
	idPlayer *player = gameLocal.GetLocalPlayer();
  //Nicemice: Needs to be fixed?!
  gameLocal.Warning("coop: idEarthQuake::Event_Activate unhandled");
	if ( !player )
		return;
...
void idEarthQuake::Think( void ) {
    ...
    //Nicemice: Needs to be fixed?!
    gameLocal.Warning("coop: idEarthQuake::Think unhandled");
    float shakeVolume = 0;
    idPlayer* player = gameLocal.GetLocalPlayer();
    if ( player )
      shakeVolume = gameSoundWorld->CurrentShakeAmplitudeForPosition( gameLocal.time, player->firstPersonViewOrigin );
...
/*
===============
idFuncRadioChatter::idFuncRadioChatter - Nicemice: modified
===============
*/
...
/*
===============
idFuncRadioChatter::ClientReceiveEvent
===============
*/
bool idFuncRadioChatter::ClientReceiveEvent(int event,int time,const idBitMsg &msg)
{
  switch (event)
  {
  case EVENT_RADIO_ACTIVATE: {
    Event_Activate(NULL);
    return true;
    }
  case EVENT_RADIO_RESET: {
    Event_ResetRadioHud(NULL);
    return true;
    }
  };
  return idEntity::ClientReceiveEvent(event,time,msg);
...
/*
================
idFuncRadioChatter::Event_Activate - Nicemice: modified
================
*/
void idFuncRadioChatter::Event_Activate( idEntity *activator ) {
  ...
  if ( gameLocal.isServer )
  {
    ServerSendEvent( EVENT_RADIO_ACTIVATE, NULL, false, -1 );
  }
  ...
  // Nicemice: Do we need an option to enable the radiochatter only for the activator?
  // currently not, this may be a feature for the post-beta phase
  ...
  shader = declManager->FindSound( sound );
    ...
    if ( player )
      ...
      // Nicemice: FIXME 
      // we need to determine the lenght of the sound shader, if we don't have a local player
      StartSoundShader( shader, SND_CHANNEL_RADIO, SSF_GLOBAL, false, &length );
    }
  ...
  PostEventSec( &EV_ResetRadioHud, time, activator );

}

/*
================
idFuncRadioChatter::Event_ResetRadioHud - Nicemice: modified
================
*/
void idFuncRadioChatter::Event_ResetRadioHud( idEntity *activator ) {
  ...
  if ( gameLocal.isServer )
  {
    ServerSendEvent( EVENT_RADIO_RESET, NULL, false, -1 );
  }
...
void idPhantomObjects::Event_Activate( idEntity *activator ) {
    ...
    //Nicemice: Needs to be fixed?!
    gameLocal.Warning("coop: idPhantomObjects::Event_Activate unhandled");
