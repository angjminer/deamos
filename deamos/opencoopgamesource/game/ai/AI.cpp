...
idAI::idAI() {
	...
	particles.Clear();
  // Nicemice: Looks like useless code to me.
	...
	headFocusRate		= 0.0f;
	focusAlignTime		= 0;
  // Nicemice: Sync the events and animations
  isServerSyncronised = true;
  fl.networkSync = true;
}
...
void idAI::Spawn( void ) {
  ...
  // Nicemice: temporary fix for the spawners
  // WE NEED A CONDITIONAL EVENT BUFFERING SYSTEM
  // TO BUFFER HIDE,SHOW AND ANIMATION EVENTS
  if (spawnArgs.GetBool("spawner"))
    Hide();
}
...
void idAI::Think( void ) {
	...
	if ( thinkFlags & TH_THINK ) {
		...
		if ( num_cinematics ) {
      if ( !IsHidden() ) {
        //gameLocal.Printf("%s %d:",name.c_str(),entityNumber );
        if ( torsoAnim.AnimDone( 0 ) ) {
          PlayCinematic();
        }
      }
			RunPhysics();
...
/*
=====================
idAI::ClientReceiveEvent - Nicemice: added
=====================
*/
...
{ 
  switch(event)
  {
  case EVENT_AI_ACTIVATE: {
    Event_Activate( NULL );
    return true;
  }
  case EVENT_AI_PREBURN: {
    Event_PreBurn();
    return true;
    }
  case EVENT_AI_BURN: {
...
/*
=====================
idAI::ClientPredictionThink - Nicemice: added
=====================
*/
void idAI::ClientPredictionThink( void )
{
    ...
    // Nicemice: FIXME: needed on clientside?
		//AI_PAIN = false;
		//AI_SPECIAL_DAMAGE = 0;
		//AI_PUSHED = false;
  } else if ( thinkFlags & TH_PHYSICS ) {
...
/*
=====================
idAI::WriteToSnapshot - Nicemice: added
=====================
*/
void idAI::WriteToSnapshot(idBitMsgDelta &msg) const
{
  ...
  msg.WriteChar ( move.moveType );
  msg.WriteFloat( ideal_yaw );
}

/*
=====================
idAI::ReadFromSnapshot - Nicemice: added
=====================
*/
void idAI::ReadFromSnapshot(const idBitMsgDelta &msg) {

  bool stateHitch = false;
  ...
  move.moveType = (moveType_t)msg.ReadChar();
...
float idAI::TravelDistance( const idVec3 &start, const idVec3 &end ) const {
	...
	if ( fromArea == toArea ) {
      ...
      gameLocal.Warning("coop: idAI::TravelDistance unhandled");
      idPlayer* player = gameLocal.GetLocalPlayer();
      if ( player )
			  gameRenderWorld->DrawText( va( "%d", ( int )dist ), ( start + end ) * 0.5f, 0.1f, colorWhite, player->viewAngles.ToMat3() );
		}
...
/*
=====================
idAI::ReactionTo - Nicemice: modified
=====================
*/
...
void idAI::Killed( idEntity *inflictor, idEntity *attacker, int damage, const idVec3 &dir, int location ) {
	...
	if ( StartRagdoll() ) {
    // Nicemice: Ragdoll stuff is also received on client
    Event_BecomeRagdoll();
...
void idAI::PlayCinematic( void ) {
  ...
  gameLocal.Printf("Playing Cinematic\n");
...
/*
=====================
idAI::Activate - Nicemice: modified
...
void idAI::Activate( idEntity *activator ) {
  ...
  {
    //Nicemice: Needs to be fixed?!
    //gameLocal.Warning("coop: idAI::Activate unhandled");
		player = gameLocal.GetLocalPlayer();
    // Nicemice: temporay fix
    if ( !player && gameLocal.isServer ) {
