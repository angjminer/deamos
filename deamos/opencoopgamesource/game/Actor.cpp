...
bool idAnimState::AnimDoneTest( int blendFrames ) const {
	int animDoneTime;

	animDoneTime = animator->CurrentAnim( channel )->GetEndTime();
  gameLocal.Printf( "%d \n",animDoneTime - gameLocal.time );
	if ( animDoneTime < 0 ) {
		// playing a cycle
		return false;
	} else if ( animDoneTime - FRAME2MS( blendFrames ) <= gameLocal.time ) {
...
void idActor::SetupHead( void ) {
    ...
    // Nicemice: If our actor is networksynchronised, lets synchronise our head too!
    if ( fl.networkSync == true )
      args.Set( "networkSync", "1" );

    // Nicemice: Since the head is spawned while map initialization, we will
    // regard it as an map entity 
    if ( isMapEntity )
      args.Set("mapEntity","1");

    // Nicemice: FIXME: Spawning heads should be only done by the server
...
/*
=====================
idActor::UpdateAnimState - Nicemice: modified
=====================
*/
void idActor::UpdateAnimState( void ) {

  // Nicemice: We are recieving the animation via network
  // so there is no need to query the scripting engine.
  if ( IsEventReceiver() )
    return;
...
/*
=====================
idActor::ClientReceiveEvent - Nicemice: added
=====================
*/
bool idActor::ClientReceiveEvent( int event, int time, const idBitMsg &msg )
{ 
  // Nicemice: Process the events which were send by the server
  switch(event)
  {
  case EVENT_ACTOR_ANIMSTATE: {
    int channel = msg.ReadShort();
    int blendFrames = msg.ReadShort();
    char statename[MAX_EVENT_PARAM_SIZE];
    msg.ReadString(statename,sizeof ( statename ) );
    const char *p = statename;
    Event_AnimState(channel,p,blendFrames);
    return true;
    }
  case EVENT_ACTOR_SETANIMPREFIX: {
    char prefix[MAX_EVENT_PARAM_SIZE];
    msg.ReadString(prefix,sizeof ( prefix ) );
    const char *p = prefix;
    Event_SetAnimPrefix(p);
    return true;
    }
  case EVENT_ACTOR_FINISHACTION: {
    char actionname[MAX_EVENT_PARAM_SIZE];
    msg.ReadString(actionname,sizeof ( actionname ) );
    const char *p = actionname;  
    Event_FinishAction(actionname);
    return true;
    }
  case EVENT_ACTOR_IDLEANIM: {
    int channel = msg.ReadShort();
    char animname[MAX_EVENT_PARAM_SIZE];
    msg.ReadString(animname,sizeof ( animname ) );
    const char *p = animname;  
    Event_IdleAnim(channel,p);
    return true;
    }
  case EVENT_ACTOR_OVERRIDEANIM: {
    int channel = msg.ReadShort();
    Event_OverrideAnim(channel);
    return true;
    }
  case EVENT_ACTOR_PLAYANIM: {
    int channel = msg.ReadShort();
    char animname[MAX_EVENT_PARAM_SIZE];
    msg.ReadString(animname,sizeof ( animname ) );
    const char *p = animname;  
    Event_PlayAnim(channel,p);
    return true;
    }
  case EVENT_ACTOR_PLAYCYCLE: {
    int channel = msg.ReadShort();
    char animname[MAX_EVENT_PARAM_SIZE];
    msg.ReadString(animname,sizeof ( animname ) );
    const char *p = animname;  
    Event_PlayCycle(channel,p);
    return true;
    }
  case EVENT_ACTOR_SETBLENDFRAMES: {
    int channel = msg.ReadShort();
    int frames = msg.ReadShort();
    Event_SetBlendFrames(channel,frames);
    return true;
    }
  case EVENT_ACTOR_STOPANIM: {
    int channel = msg.ReadShort();
    int frames = msg.ReadShort();
    Event_StopAnim(channel,frames);
    return true;
    }
  }
  
  return idAFEntity_Gibbable::ClientReceiveEvent(event,time,msg);
}
...
void idActor::Event_EnableEyeFocus( void ) {
  ...
  //  gameLocal.Printf("idActor::Event_EnableEyeFocus: %s(%s)\n",GetType()->classname,name.c_str());
...
void idActor::Event_DisableEyeFocus( void ) {
  ...
  // Nicemice: DEBUG
  //if ( isServerSyncronised )
  //  gameLocal.Printf("idActor::Event_DisableEyeFocus: %s(%s)\n",GetType()->classname,name.c_str());
...
void idActor::Event_EnableWalkIK( void ) {
  ...
  // Nicemice: DEBUG
  //if ( isServerSyncronised )
  //  gameLocal.Printf("idActor::Event_EnableWalkIK: %s(%s)\n",GetType()->classname,name.c_str());
...
void idActor::Event_DisableWalkIK( void ) {
  ...
  // Nicemice: DEBUG
  //if ( isServerSyncronised )
  //  gameLocal.Printf("idActor::Event_DisableWalkIK: %s(%s)\n",GetType()->classname,name.c_str());
...
void idActor::Event_EnableLegIK( int num ) {
  ...
  // Nicemice: DEBUG
  if ( isServerSyncronised )
    gameLocal.Printf("idActor::Event_EnableLegIK: %s(%s)\n",GetType()->classname,name.c_str());
...
void idActor::Event_DisableLegIK( int num ) {
  ...
  // Nicemice: DEBUG
  //if ( isServerSyncronised )
  //  gameLocal.Printf("idActor::Event_DisableLegIK: %s(%s)\n",GetType()->classname,name.c_str());
...
void idActor::Event_SetAnimPrefix( const char *prefix ) {
  ...
  if ( IsEventSender() )
  {
    ...
    ServerSendEvent( EVENT_ACTOR_SETANIMPREFIX, &msg, false, gameLocal.localClientNum );
  }

  // Nicemice: DEBUG
  //if ( isServerSyncronised )
  //  gameLocal.Printf("idActor::Event_SetAnimPrefix: %s(%s)\n",GetType()->classname,name.c_str());
...
/*
===============
idActor::Event_StopAnim - Nicemice: modified
===============
*/
void idActor::Event_StopAnim( int channel, int frames ) {
  ...
  if ( IsEventSender() )
  {
    ...
    msg.WriteShort(frames);
    ServerSendEvent( EVENT_ACTOR_STOPANIM, &msg, false , gameLocal.localClientNum );
  }

  // Nicemice: DEBUG
  //if ( isServerSyncronised )
  //  gameLocal.Printf("idActor::Event_StopAnim: %s(%s)\n",GetType()->classname,name.c_str());
...
/*
===============
idActor::Event_PlayAnim - Nicemice: modified
===============
*/
void idActor::Event_PlayAnim( int channel, const char *animname ) {
  ...
  if ( IsEventSender() )
  {
    ...
    msg.WriteShort(channel);
    msg.WriteString(animname);
    ServerSendEvent( EVENT_ACTOR_PLAYANIM, &msg, false, -1 );
  }

  // Nicemice: DEBUG
  //if ( isServerSyncronised )
  //  gameLocal.Printf("idActor::Event_PlayAnim: %s(%s)\n",GetType()->classname,name.c_str());
...
/*
===============
idActor::Event_PlayCycle - Nicemice: modified
===============
*/
void idActor::Event_PlayCycle( int channel, const char *animname ) {
  ...
  if ( IsEventSender() )
  {
    ...
    msg.WriteShort(channel);
    msg.WriteString(animname);
    ServerSendEvent( EVENT_ACTOR_PLAYCYCLE, &msg, false, gameLocal.localClientNum );

  }

  // Nicemice: DEBUG
  //if ( isServerSyncronised )
  //  gameLocal.Printf("idActor::Event_PlayCycle: %s(%s)\n",GetType()->classname,name.c_str());
...
/*
===============
idActor::Event_IdleAnim - Nicemice: modified
===============
*/
void idActor::Event_IdleAnim( int channel, const char *animname ) {
  ...
  if ( IsEventSender() )
  {
    ...
    msg.WriteShort(channel);
    msg.WriteString(animname);
    ServerSendEvent( EVENT_ACTOR_IDLEANIM, &msg, false, gameLocal.localClientNum );
  }

  // Nicemice: DEBUG
  //if ( isServerSyncronised )
  //  gameLocal.Printf("idActor::Event_IdleAnim: %s(%s)\n",GetType()->classname,name.c_str());
...
void idActor::Event_SetSyncedAnimWeight( int channel, int anim, float weight ) {
	...
	// Nicemice: DEBUG
  //if ( isServerSyncronised )
  //  gameLocal.Printf("idActor::Event_SetSyncedAnimWeight: %s(%s)\n",GetType()->classname,name.c_str());
...
/*
===============
idActor::Event_OverrideAnim - Nicemice: modified
===============
*/
void idActor::Event_OverrideAnim( int channel ) {
  ...
  if ( IsEventSender() )
  {
    ...
    msg.WriteShort(channel);
    ServerSendEvent( EVENT_ACTOR_OVERRIDEANIM, &msg, false , gameLocal.localClientNum );

  }

  // Nicemice: DEBUG
  //if ( isServerSyncronised )
  //  gameLocal.Printf("idActor::Event_OverrideAnim: %s(%s)\n",GetType()->classname,name.c_str());
...
void idActor::Event_EnableAnim( int channel, int blendFrames ) {
  ...
  // Nicemice: DEBUG
  if ( isServerSyncronised )
    gameLocal.Printf("idActor::Event_EnableAnim: %s(%s)\n",GetType()->classname,name.c_str());
...
/*
===============
idActor::Event_SetBlendFrames - Nicemice: modified
===============
*/
void idActor::Event_SetBlendFrames( int channel, int blendFrames ) {
  ...
  if ( IsEventSender() )
  {
    ...
    msg.WriteShort(channel);
    msg.WriteShort(blendFrames);
    ServerSendEvent( EVENT_ACTOR_SETBLENDFRAMES, &msg, false, gameLocal.localClientNum );

  }

  // Nicemice: DEBUG
  //if ( isServerSyncronised )
  //  gameLocal.Printf("idActor::Event_SetBlendFrames: %s(%s)\n",GetType()->classname,name.c_str());
...
/*
===============
idActor::Event_AnimState - Nicemice: modified
===============
*/
void idActor::Event_AnimState( int channel, const char *statename, int blendFrames ) {
  ...
  if ( IsEventSender() )
  {
    ...
    msg.WriteShort(channel);
    msg.WriteShort(blendFrames);
    msg.WriteString(statename);
    ServerSendEvent( EVENT_ACTOR_ANIMSTATE, &msg, false, gameLocal.localClientNum );

  }

  // Nicemice: DEBUG
  //if ( isServerSyncronised )
  //  gameLocal.Printf("idActor::Event_AnimState: %s(%s)\n",GetType()->classname,name.c_str());
...
/*
===============
idActor::Event_FinishAction - Nicemice: modified
===============
*/
void idActor::Event_FinishAction( const char *actionname ) {
  ...
  if ( IsEventSender() )
  {
    ...
    msg.WriteString(actionname);

    ServerSendEvent( EVENT_ACTOR_FINISHACTION, &msg, false, gameLocal.localClientNum );

  }

  // Nicemice: DEBUG
  //if ( isServerSyncronised )
  //  gameLocal.Printf("idActor::Event_FinishAction: %s(%s)\n",GetType()->classname,name.c_str());
