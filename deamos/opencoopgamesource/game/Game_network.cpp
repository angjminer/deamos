...
void idGameLocal::ServerWriteSnapshot( int clientNum, int sequence, idBitMsg &msg, byte *clientInPVS, int numPVSClients ) {
    ...
    // Nicemice: TEST TEST - RequestSnapshot is deprecated
		if ( !ent->PhysicsTeamInPVS( pvsHandle ) && ent->entityNumber != clientNum && !ent->RequestSnapshot(clientNum)) {
			continue;
		}
      ...
      /*gameLocal.DPrintf("NOSYNC: ");
      gameLocal.DPrintf("Ent: %i %s %s\n",ent->entityNumber,ent->name.c_str(),ent->GetType()->classname);*/
			continue;
		}

    // Nicemice: DEBUG SYNC
    /*gameLocal.DPrintf("  SYNC: ");
    gameLocal.DPrintf("Ent: %i %s %s\n",ent->entityNumber,ent->name.c_str(),ent->GetType()->classname);*/
...
void idGameLocal::ServerProcessReliableMessage( int clientNum, const idBitMsg &msg ) {
	...
	switch( id ) {
      ...
      // Nicemice: TEST TEST
      eventQueue.Enqueue( event, idEventQueue::OUTOFORDER_SORT );
  ...
  //gameLocal.DPrintf("----------------------------\n");
	...
	for( ent = spawnedEntities.Next(); ent != NULL; ent = ent->spawnNode.Next() ) {
		...
		if ( !( snapshot->pvs[ent->entityNumber >> 5] & ( 1 << ( ent->entityNumber & 31 ) ) ) ) {
          ...
          // Nicemice: this is currently on testing, lets see if it works out
          // since entities may be moved on the server while not being in the pvs of the client
          // he will produce the stale message because is still sees it when moving back into 
          // this pvs. To put the entity right back into the correct PVS we request a snapshot
          // message from the server.
          /*if ( ent->RequestSnapshot(clientNum) ) {
            gameLocal.Printf("coop: requesting snapshot for: %s(%s)\n",ent->GetType()->classname,ent->name.c_str());
          } else {
            gameLocal.Printf("client thinks map entity %s(%s) is stale\n",ent->GetType()->classname,ent->name.c_str());
          }*/
          if ( clientEntityStates[clientNum][ent->entityNumber] ) {
            ent->Hide();
            if ( developer.GetBool() ) {
              gameRenderWorld->DrawText( "stale", ent->GetPhysics()->GetAbsBounds().GetCenter(), 1.0f, colorRed, gameLocal.GetLocalPlayer()->viewAngles.ToMat3(), 1, SEC2MS(0.5f) );
            }
            gameLocal.Warning("map entity %s(%s) is stale",ent->GetType()->classname,ent->name.c_str());
          }
				} else {
					ent->FreeModelDef();
					ent->UpdateVisuals();
    ...
    // Nicemice: TEST TEST
    ent->ResetSnapshotRequest( clientNum );
		...
		if ( !typeInfo || ent->GetType()->typeNum != typeNum || ent->entityDefNumber != entityDefNumber ) {
			...
			common->Warning( "entity is not the right type %p 0x%d 0x%x 0x%x 0x%x", typeInfo, ent->GetType()->typeNum, typeNum, ent->entityDefNumber, entityDefNumber );
			continue;
		}
...
void idGameLocal::ClientProcessEntityNetworkEventQueue( void ) {
	...
	while( eventQueue.Start() ) {
		...
		if( !entPtr.SetSpawnId( event->spawnId ) ) {
			...
			eventMsg.BeginReading();
		  // Nicemice: DEBUG
		  //gameLocal.Printf( "Event: %i %i %s(%s)\n",event->event,ent->entityNumber,ent->GetType()->classname,ent->name.c_str());
