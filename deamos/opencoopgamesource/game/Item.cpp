...
/*
================
idItem::Spawn - modified by Nicemice
================
*/
void idItem::Spawn( void ) {
  ...
  // Nicemice: removed item spining in multiplayer
...
bool idObjective::ClientReceiveEvent(int event,int time,const idBitMsg &msg)
{
  ...
  case EVENT_HIDEOBJECTIVE: {
    Event_HideObjective( NULL );
    return true;
    }
  case EVENT_TRIGGER: {
    Event_Trigger( NULL );
...
void idObjective::Event_Trigger( idEntity *activator ) {
	...
	for( int i = 0; i < gameLocal.num_entities; i++ )
  {
      ...
      continue;
    if ( !gameLocal.entities[ i ]->IsType( idObjectiveComplete::Type ) ) 
      continue;
...
void idObjective::Event_GetPlayerPos() {
  if ( !gameLocal.isMultiplayer ) {
    ...
    // Nicemice: just hide the objective after 5 seconds
    PostEventMS( &EV_HideObjective, 5000, NULL );
  }
}
...
/*
================
idPDAItem::Event_GivePDA
================
*/
void idPDAItem::Event_GivePDA(idPlayer *player)
{
  if ( gameLocal.isServer )
  {
    idBitMsg	msg;
  ...
  // Nicemice: DEBUG
  //gameLocal.DPrintf("idPDAItem::Event_GivePDA\n");
...
/*
================
idMoveableItem::DropItem - Nicemice: modified
================
*/
...
bool idMoveablePDAItem::ClientReceiveEvent(int event,int time,const idBitMsg &msg)
{
  switch (event)
  {
  case EVENT_GIVEPDA: {
  ...
  return idMoveableItem::ClientReceiveEvent(event,time,msg);
}
...
/*
================
idObjectiveComplete::idObjectiveComplete - Nicemice: modified
================
*/
...
/*
================
idObjectiveComplete::ClientReceiveEvent - Nicemice: added
================
*/
bool idObjectiveComplete::ClientReceiveEvent( int event, int time, const idBitMsg &msg ) {
  ...
  return idItemRemover::ClientReceiveEvent( event, time, msg );
}
...
/*
================
idObjectiveComplete::Event_GetPlayerPos - Nicemice: modified
================
*/
