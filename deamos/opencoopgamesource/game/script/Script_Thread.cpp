...
void idThread::Event_Trigger( idEntity *ent ) {
  ...
  // Nicemice: allow triggering events on the client?
  ...
  if ( gameLocal.isServer ) {
    if ( ent->fl.networkSync == false && ent->NetworkSyncTarget() ) {
      // Nicemice: Good idea to enable networksync while ingame?
      //ent->fl.networkSync = true;
      //gameLocal.DPrintf("coop: Adding networkSync to %s(%s)\n",ent->GetType()->classname,ent->name.c_str());
      gameLocal.Warning("entity %s(%s) triggered by script is not networksynced",ent->GetType()->classname,ent->name.c_str());
    }
  }
...
/*
================
idThread::Event_SetCamera - Nicemice: modified
================
*/
void idThread::Event_SetCamera( idEntity *ent ) {
  ...
  // Nicemice: DEBUG
  // gameLocal.DPrintf("idThread::Event_SetCamera called\n");

  // Nicemice: modified
  (static_cast<idCamera*>(ent))->Event_SetCamera();
  //gameLocal.SetCamera((idCamera*)ent);
}

/*
================
idThread::Event_FirstPerson - Nicemice: modified
================
*/
void idThread::Event_FirstPerson( void ) {

  // Nicemice: DEBUG
  // gameLocal.Printf("idThread::Event_FirstPerson called\n");

  idCamera* cam = gameLocal.GetCamera();
  if ( cam ) {
    cam->Event_ResetCamera();
  }
...
void idThread::Event_FadeIn( idVec3 &color, float time ) {
  ...
  gameLocal.Warning("coop: idThread::Event_FadeIn unhandled");
...
void idThread::Event_FadeOut( idVec3 &color, float time ) {
  ...
  //Nicemice: Needs to be fixed?!
  gameLocal.Warning("coop: idThread::Event_FadeOut unhandled");
...
void idThread::Event_FadeTo( idVec3 &color, float alpha, float time ) {
  ...
  //Nicemice: Needs to be fixed?!
  gameLocal.Warning("coop: idThread::Event_FadeTo unhandled");
	player = gameLocal.GetLocalPlayer();
	if ( player ) {
...
void idThread::Event_DrawText( const char *text, const idVec3 &origin, float scale, const idVec3 &color, const int align, const float lifetime ) {
  //Nicemice: Needs to be fixed?!
  gameLocal.Warning("coop: idThread::Event_DrawText unhandled");
...
void idThread::Event_InfluenceActive( void ) {
	idPlayer *player;
  //Nicemice: Needs to be fixed!
  //gameLocal.Warning("coop: idThread::Event_InfluenceActive unhandled");
	player = gameLocal.GetLocalPlayer();
