...
/*
=====================
idCamera::Event_SetCamera - Nicemice: added
=====================
*/
void idCamera::Event_SetCamera() {

  if ( gameLocal.isServer )
  {
    ServerSendEvent( EVENT_CAMERA_SETCAMERA, NULL, false, -1);
  }
...
/*
=====================
idCamera::Event_ResetCamera - Nicemice: added
=====================
*/
void idCamera::Event_ResetCamera() {

  if ( gameLocal.isServer )
  {
    ServerSendEvent( EVENT_CAMERA_RESETCAMERA, NULL, false, -1);
  }
...
/*
=====================
idCamera::ClientReceiveEvent - Nicemice: modified
=====================
*/
bool idCamera::ClientReceiveEvent(int event,int time,const idBitMsg &msg)
{
  switch(event)
  {
  case EVENT_CAMERA_SETCAMERA: {
    Event_SetCamera();
    return true;
    }
  case EVENT_CAMERA_RESETCAMERA: {
    Event_ResetCamera();
    return true;
    }
  }
...
/*
===============
idCameraView::Event_SetAttachments - Nicemice: modified
================
*/
void idCameraView::Event_SetAttachments(  ) {

  // Nicemice: do we need to send this event?
  /*if ( gameLocal.isServer ) {
    ServerSendEvent( EVENT_CVIEW_SETATTACHMENTS, NULL, false, -1 );
  }*/
...
void idCameraView::Event_Activate( idEntity *activator ) {
  ...
  // Nicemice: DEBUG
  //gameLocal.DPrintf("idCameraView::Event_Activate\n");
...
void idCameraAnim::Start( void ) {
  ...
  idPlayer *player = gameLocal.GetLocalPlayer();
  if ( player ) {
	  if ( player->GetRenderView()->time == gameLocal.time ) {
...
/*
===============
idCameraAnim::ClientPredictionThink
================
*/
void idCameraAnim::ClientPredictionThink( void )
{
...
bool idCameraAnim::ClientReceiveEvent(int event,int time,const idBitMsg &msg)
{
  switch (event)
  {
  case EVENT_CANIM_ACTIVATE: {
    Event_Activate( NULL );
    return true;
    }
  case EVENT_CANIM_START: {
    Event_Start();
    return true;
    }
  case EVENT_CANIM_STOP: {
    Event_Stop();
    return true;
    }
  };

  return idCamera::ClientReceiveEvent(event,time,msg);
}
...
void idCameraAnim::Event_Activate( idEntity *_activator ) {
  ...
  if ( gameLocal.isServer )
  {
    ServerSendEvent( EVENT_CANIM_ACTIVATE, NULL, false, -1 );
  }

  // Nicemice: DEBUG
  //gameLocal.DPrintf("idCameraAnim::Event_Activate\n");
...
void idCameraAnim::Event_Start( void ) {
  ...
  if ( gameLocal.isServer ) {
    ServerSendEvent( EVENT_CANIM_START, NULL, false, -1);
  }

  // Nicemice: DEBUG
  //gameLocal.DPrintf("idCameraAnim::Event_Start\n");
...
void idCameraAnim::Event_Stop( void ) {
  ...
  if ( gameLocal.isServer ) {
    ServerSendEvent( EVENT_CANIM_STOP, NULL, false, -1);
  }

  // Nicemice: DEBUG
  //gameLocal.DPrintf("idCameraAnim::Event_Stop\n");
